...
def run_test_case_3(f):
  ...
  print("\u2705 `df` looks good with \"sample.png\"!")
  ...
  pixels = len(df)
  if pixels != 150 * 150:
    print(f"\N{CROSS MARK} `df` must contain one entry for every pixel.  In the \"sample-50x.png\" image, you only had {pixels} when you should have had 22500.")
    return

  print("\u2705 `df` looks good with \"sample-50x.png\"!")
  print(f"{tada} All tests passed! {tada}")
